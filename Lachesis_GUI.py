...
"""}
        ...
        answers_df['Необработанный_результат'] = answers_df.apply(processing_result_ddo, axis=1)
        answers_df['Необработанный_результат'] = answers_df['Необработанный_результат'].astype(str)

        answers_df['Обработанный_результат'] = answers_df.apply(processing_finish_result_ddo, axis=1)
        answers_df['Обработанный_результат'] = answers_df['Обработанный_результат'].astype(str)
        answers_df['Описание_результата'] = answers_df['Обработанный_результат'].apply(create_out_str_ddo)
        ...
        if 'ФИО' in df.columns:
            ...
            df['Описание_результата'] = 'Дифференциально-диагностический опросник \n' + df['Описание_результата']
        ...
        send_df['Необработанный_результат'] = df['Необработанный_результат']
...
def create_out_str_ddo(x):
    ...
    """
    return f'{dct_desciprion.get(x, "Проверьте результаты в колонке Результат")}\nРекомендуемые профессии:\n{dct_prof.get(x, "Проверьте результаты в колонке Результат")}'
...
def processing_optl():
    ...
    try:
        ...
        answers_df = answers_df.applymap(lambda x: x.strip())
        answers_df['Результат'] = answers_df.apply(processing_result_optl, axis=1)
        answers_df['Результат'] = answers_df['Результат'].astype(str)
...
"""
        ...
        if 'ФИО' in df.columns:
            ...
            df['Описание_результата'] = 'Определение профессионального типа личности\n' + df['Результат'] + description_result
...
def processing_dcok():
    ...
    try:
        ...
        answers_df['Необработанный_результат'] = answers_df.apply(processing_result_dcok, axis=1)
        answers_df['Необработанный_результат'] = answers_df['Необработанный_результат'].astype(str)

        # Создаем колонку для итоговой обработки
        answers_df['Обработанный_результат'] = answers_df.apply(processing_finish_result_dcok, axis=1)
        answers_df['Обработанный_результат'] = answers_df['Обработанный_результат'].astype(str)
...
"""
        ...
        if 'ФИО' in df.columns:
            df['Описание_результата'] = df['ФИО'] + ' \nДиагностика ценностных ориентаций в карьере\n' + df[
                'Обработанный_результат'] + description_result
        else:
            df['Описание_результата'] = 'Диагностика ценностных ориентаций в карьере\n' + df[
                'Обработанный_результат'] + description_result
...
"""
        ...
        if 'ФИО' in sep_dcok_df.columns:
            sep_dcok_df['ДЦОК_Описание_результата'] = sep_dcok_df[
                                                          'ФИО'] + ' \nДиагностика ценностных ориентаций в карьере\n' + \
...
"""
        ...
        if 'ФИО' in sep_optl_df.columns:
            sep_optl_df['ОПТЛ_Описание_результата'] = sep_optl_df[
                                                          'ФИО'] + ' \nОпределение профессионального типа личности\n' + \
...
"""
        ...
        if 'ФИО' in sep_sppu_df.columns:
            sep_sppu_df['СППУ_Описание_результата'] = sep_sppu_df[
                                                          'ФИО'] + ' \nСфера профессиональных предпочтений учащихся\n' + \
