...
def processing_ddo():
    ...
    try:
        ...
        answers_df['Результат'] = answers_df.apply(processing_result_ddo, axis=1)
        answers_df['Результат'] = answers_df['Результат'].astype(str)
        answers_df['Описание_результата'] = answers_df['Результат'].apply(create_out_str_ddo)
...
"""
        ...
        wb.save(f'{path_to_end_folder_sppu}/Краткая таблица с результатами СППУ  от {current_time}.xlsx')
    except NameError:
        messagebox.showerror('Лахезис Обработка результатов профориентационных тестов ver 1.1',
                             f'Выберите файлы с данными и папку куда будет генерироваться файл')
    except KeyError as e:
...
"""
        ...
        wb.save(f'{path_to_end_folder_optl}/Краткая таблица с результатами ОТПЛ  от {current_time}.xlsx')
    except NameError:
        messagebox.showerror('Лахезис Обработка результатов профориентационных тестов ver 1.1',
                             f'Выберите файлы с данными и папку куда будет генерироваться файл')
    except KeyError as e:
...
9) Предпринимательство – Создавать новые организации, товары, услуги.
 Этим людям нравится создавать новые организации, товары или услуги, которые могут быть отождествлены с их усилиями. Работать на других – это не их, они – предприниматели по духу, и цель их карьеры – создать что-то новое, организовать свое дело, воплотить в жизнь идею, всецело принадлежащую только им. Вершина карьеры в их понимании – собственный бизнес.
        ...
        wb.save(f'{path_to_end_folder_dcok}/Краткая таблица с результатами ДЦОК  от {current_time}.xlsx')
