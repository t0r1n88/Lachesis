...
def processing_ntfp(base_df: pd.DataFrame, answers_df: pd.DataFrame):
    ...
    try:
        ...
        lst_check_cols = ['В любой работе мне важны стабильность, предсказуемость','Я желаю сам выбирать, какие именно рабочие заказы буду выполнять',
                          'Я хочу иметь свое собственное дело, а не работать на кого-то','Я считаю, что важная составляющая работы — социальные гарантии (выплата больничных, будущая пенсия и т. п.)',
                          'Я согласен, что надо мной будет начальство, но хотел бы сам выбирать, под чьим именно руководством трудиться','Терпеть не могу, когда кто-то указывает мне, как нужно делать работу',
                          'Хочется четко представлять свои функциональные обязанности, всегда знать, что именно я должен делать','Сделать карьеру для меня — это не самому стать начальником, а быть таким специалистом, за которым начальники будут охотиться и сами предлагать хорошие деньги',
                          'Для меня принципиально важно, чтобы надо мной не было начальников; в работе хочу быть предоставлен самому себе','Готов смириться с относительно меньшими перспективами карьерного роста в обмен на то, что моя работа будет спокойной и стабильной',
                          'Я готов работать в режиме резкого изменения загруженности: «то густо, то пусто»','Для меня принципиально важно воплощать в жизнь то, что я задумал, пусть даже на это потребуется гораздо больше времени и сил, чем предполагает обычный рабочий день',
                          'Я предпочел бы, чтобы не я сам, а работодатель заботился о поиске заказов и о том, как организовать мой труд','Согласен, что содержание и график моей работы могут резко меняться в зависимости от того, какой заказ удалось получить',
                          'Я готов вкладывать в развитие дела личные финансовые средства, даже если рискую их потерять','Я готов вести себя в четком соответствии с теми правилами, которые заданы моим работодателем',
                          'Я согласен обойтись без постоянной гарантированной зарплаты, а получать деньги лишь за то, что фактически выполнил','Полагаю, что лучше рисковать — «либо заработаю много, либо потеряю все», — нежели довольствоваться небольшой, но гарантированной зарплатой',
                          'Я не желаю заниматься чем-либо, выходящим за рамки моей основной работы','Я готов смириться с тем, что у меня долго вообще не будет заработков, если не удалось найти заказы',
                          'Полагаю, что у меня хорошие способности к управлению другими людьми и решению неожиданных проблем','Для меня предпочтительнее работать за гарантированную зарплату, а не подвергать себя рискам, связанным с собственным бизнесом',
                          'Готов работать на начальника, но не согласен, чтобы он «монополизировал» меня; пусть лучше у меня будет сразу несколько начальников','Считаю, что иметь собственный бизнес — это важный показатель жизненного успеха человека'
                          ]
...
Психологические тесты для старшеклассников и студентов. —СПб.: Питер, 2012. ISBN 978-5-459-01074-9
        ...
        """
                       Своды 
                       """
        lst_reindex_group_cols = ['Класс', 'низкая', 'ниже среднего',
                 'средняя',
                 'выше среднего', 'высокая', 'Итого']
        lst_reindex_group_sex_cols = ['Класс', 'Пол', 'низкая', 'ниже среднего',
                 'средняя',
                 'выше среднего', 'высокая', 'Итого']

        lst_reindex_course_cols = ['Номер_класса', 'низкая', 'ниже среднего',
                 'средняя',
                 'выше среднего', 'высокая', 'Итого']
        lst_reindex_course_sex_cols = ['Номер_класса', 'Пол', 'низкая', 'ниже среднего',
                 'средняя',
                 'выше среднего', 'высокая', 'Итого']
