"""
Скрипт для обработки результатов Диагностический опросник для выявления склонности к различным формам девиантного поведения для студентов СПО
"""
...
class BadOrderDAPPSPO(Exception):
    """
...
class BadValueDAPPSPO(Exception):
    """
    Исключение для неправильных значений в вариантах ответов
    """
    pass

class BadCountColumnsDAPPSPO(Exception):
    """
...
def calc_value_adp(row):
    ...
    :return: число
    """
    lst_pr = [4,8,12,13,15,20,26,28,31,
              34,35,37,38,41,45,47,48,52,53,55,56,60]
...
def calc_value_dp(row):
    ...
    :return: число
    """
    lst_pr = [7,10,14,17,19,21,22,23,
              25,27,32,43,50,51,57,58]
    value_forward = 0  # результат
    for idx, value in enumerate(row,1):
        if idx in lst_pr:
            value_forward += value
...
def calc_value_sr(row):
    ...
    :return: число
    """
    lst_pr = [2,5,11,24,30,36,42,44,54,59]
    value_forward = 0  # результат
    for idx, value in enumerate(row,1):
        if idx in lst_pr:
            value_forward += value
...
def calc_value_pn(row):
    """
    Функция для подсчета значения
    :return: число
    """
    lst_pr = [3,6,9,16,18,29,33,39,40,46,49]
    value_forward = 0  # результат
    for idx, value in enumerate(row,1):
        if idx in lst_pr:
            value_forward += value
...
def create_list_on_level_dapp_spo(base_df:pd.DataFrame, out_dct:dict, lst_level:list, dct_prefix:dict):
    """
...
def create_result_dapp_spo(base_df:pd.DataFrame, out_dct:dict, lst_svod_cols:list):
    """
    ...
    # ПН
    lst_pn = ['9-10', '3-8', '1-2']
    lst_reindex_one_level_pn_cols = lst_svod_cols.copy()
    lst_reindex_one_level_pn_cols.extend(['9-10', '3-8', '1-2',
                                       'Итого'])  # Основная шкала
    svod_count_one_level_pn_df = calc_count_scale(base_df, lst_svod_cols,
                                                  'ПН_Стен',
                                                  'ПН_Диапазон',
                                                  lst_reindex_one_level_pn_cols, lst_pn)
    ...
    dct_rename_cols_mean = {'ИП_Значение': 'Ср. Интегральный показатель',
                            ...
                            'ПН_Значение': 'Ср. Профессиональная направленность',
                            }
    ...
    out_dct.update({f'Ср. {out_name}': svod_mean_one_df,
                    ...
                    f'ПН {out_name}': svod_count_one_level_pn_df
                    })
    ...
    if len(lst_svod_cols) == 1:
        ...
        for idx, name_column in enumerate(lst_svod_cols):
            ...
            lst_reindex_column_level_pn_cols = [lst_svod_cols[idx], '9-10', '3-8', '1-2',
                                             'Итого']
            ...
            svod_count_column_level_pn_df = calc_count_scale(base_df, lst_svod_cols,
                                                          'ПН_Стен',
                                                          'ПН_Диапазон',
                                                          lst_reindex_column_level_pn_cols, lst_pn)
            ...
            out_dct.update({f'Ср. {name_column}': svod_mean_column_df,
                            ...
                            f'ПН {name_column}': svod_count_column_level_pn_df,
                            })
        return out_dct
...
def processing_dap_p_spo(base_df: pd.DataFrame, answers_df: pd.DataFrame, lst_svod_cols:list):
    """
    ...
    try:
        ...
        lst_check_cols = ['Я хорошо понял инструкцию к данной методике',
                          ...
                          'Я совершенно не приспособлен к будущей профессии',
                          ...
                          'Главная причина моего поступления в лицей, техникум – мне просто некуда больше пойти',
                          ...
                          'Мне не нравится моя специальность',
                          'В состоянии агрессии я способен(а) на многое',
                          'Я не собираюсь работать по профессии после окончания лицея, техникума',
                          'Среди моих близких родственников (отец, мать, братья, сестры) были судимые лица',
                          'Я часто испытываю чувство невесомости тела, отрешенности от окружающего мира, нереальности происходящего',
                          ...
                          'Я не осуждаю людей, которые учатся только ради «корочек», а потом пойдут на более удобную и «чистую» работу',
                          'Смысл жизни не всегда бывает ясен, иногда его можно потерять',

                          'У менять есть друзья, которые любят смотреть «мультики» после приема разных веществ',
                          'В районе, где я проживаю, есть молодежные тусовки, которые активно враждуют между собой',
                          'Я бы остался в школе и получил более престижное образование, но у меня нет такой возможности',
                          ...
                          'Вряд ли я захочу всю жизнь работать по выбранной специальности',
                          ...
                          'В нашей школе был принят «ритуал прописки» новичков, и я в нем активно участвовал',
                          ...
                          'Во время обучения профессии я не собираюсь слишком «напрягаться», мне достаточно самых основных навыков и умений',
                          ...
                          'Я всегда хотел продолжить семейную династию и выбрать именно эту профессию',
                          ...
                          'Я совершенно не приспособлен к будущей профессии',
                          'Мои близкие друзья частенько уклонялись от учебных занятий (воспитательных мероприятий), считая их скучными, неинтересными',
        ...
        base_df['ПН_Значение'] = answers_df.apply(calc_value_pn, axis=1)
        base_df['ПН_Стен'] = base_df['ПН_Значение'].apply(calc_pn_sten)
        base_df['ПН_Диапазон'] = base_df['ПН_Стен'].apply(calc_range)
        ...
        new_order_lst = ['Итоговые_стены', 'ИП_Стен', 'АДП_Стен',
                         'ДП_Стен', 'СР_Стен','ПН_Стен',

                         'ИП_Уровень', 'АДП_Уровень', 'ДП_Уровень', 'СР_Уровень','ПН_Диапазон',

                         'ИП_Значение', 'АДП_Значение', 'ДП_Значение', 'СР_Значение','ПН_Значение'
                         ]
        ...
        part_df['ДАППВМА_ПН_Стен'] = base_df['ПН_Стен']
        ...
        part_df['ДАППВМА_ПН_Диапазон'] = base_df['ПН_Диапазон']
        ...
        part_df['ДАППВМА_ПН_Значение'] = base_df['ПН_Значение']
        ...
        # Делаем свод  по  шкалам
        dct_svod_pn = {'ПН_Стен': 'ПН_Диапазон',
                        }
        dct_rename_svod_pn = {
                               'ПН_Стен': 'Профессиональная направленность',
                               }

        lst_pn = ['9-10', '3-8', '1-2']

        base_svod_pn_df = create_union_svod(base_df, dct_svod_pn, dct_rename_svod_pn, lst_pn)
        ...
        avg_pn = round(base_df['ПН_Значение'].mean(), 2)
        ...
        avg_dct = {'Среднее значение интегрального показателя': avg_ip,
                   ...
                   'Среднее значение шкалы Профессиональная направленность': avg_pn,
                   }
        ...
        out_dct = create_list_on_level_dapp_spo(base_df, out_dct, lst_sub, dct_prefix)

        dct_pn = {'ПН_Диапазон':'ПН'}
        out_dct = create_list_on_level(base_df,out_dct,lst_pn,dct_pn)
        """
        ...
        if len(lst_svod_cols) == 0:
            ...
            out_dct = create_result_dapp_spo(base_df, out_dct, lst_svod_cols)
            return out_dct, part_df


    except BadOrderDAPPSPO:
        messagebox.showerror('Лахеcис',
                             f'При обработке вопросов теста ДАП-П для студентов СПО обнаружены неправильные вопросы. Проверьте названия колонок с вопросами:\n'
                             f'{error_order_message[:5000]}\n'
                             f'Используйте при создании Яндекс-формы написание вопросов из руководства пользователя программы Лахесис.')
    except BadValueDAPPSPO:
        messagebox.showerror('Лахеcис',
                             f'При обработке вопросов теста ДАП-П для студентов СПО обнаружены неправильные варианты ответов. Проверьте ответы на указанных строках:\n'
                             f'{error_message[:5000]}\n'
                             f'Используйте при создании Яндекс-формы написание вариантов ответа из руководства пользователя программы Лахесис.')


    except BadCountColumnsDAPPSPO:
        messagebox.showerror('Лахеcис',
                             f'Проверьте количество колонок с ответами на тест ДАП-П для студентов СПО\n'
