"""
Скрипт для обработки результатов Скрининговая методика оценки личностных ресурсов обучающихся Васягина, Григорян, Баринова форма 2 для 9-11 классов, студентов
"""
...
class BadOrderOLROVST(Exception):
    """
...
class BadValueOLROVST(Exception):
    """
    Исключение для неправильных значений в вариантах ответов
    """
    pass

class BadCountColumnsOLROVST(Exception):
    """
    Исключение для обработки случая если количество колонок не равно 51
    """
    pass
...
def calc_value_lr(row):
    ...
    :return: число
    """
    lst_pr = [4, 10, 14, 17, 20, 24, 25, 26, 28, 29, 33, 38, 46, 50, 51,
              1, 2, 3, 5, 7, 8, 9, 11, 12, 15, 16, 19, 21, 22, 23, 30, 31,
              32, 34, 35, 36, 37, 39, 41, 42, 43, 44, 45, 48, 49]

    lst_neg = [1, 2, 3, 5, 7, 8, 9, 11, 12, 15, 16, 19, 21, 22, 23, 30, 31,
              32, 34, 35, 36, 37, 39, 41, 42, 43, 44, 45, 48, 49]
    value_forward = 0  # результат
    for idx, value in enumerate(row, 1):
        if idx in lst_pr:
...
def create_result_orlo_vas_st(base_df:pd.DataFrame, out_dct:dict, lst_svod_cols:list):
    """
...
def processing_olro_vas_star(base_df: pd.DataFrame, answers_df: pd.DataFrame, lst_svod_cols:list):
    """
    ...
    try:
        ...
        if len(answers_df.columns) != 51:  # проверяем количество колонок с вопросами
            raise BadCountColumnsOLROVST
        ...
        lst_check_cols = ['Интересным, но трудно выполнимым задачам я предпочитаю те, с которыми ранее удавалось справиться',
                          'Я веду себя не так, как хочу, а так, как ждут от меня окружающие',
                          ...
                          'Я стараюсь быть в курсе событий, происходящих вокруг меня',
                          'Я никогда не вру',
                          ...
                          'Если человеку скучно, значит он не умеет видеть интересное',
                          ...
                          'У меня есть уверенность, что результат происходящего зависит от меня',
                          ...
                          'Я с трудом что сближаюсь с другими людьми',
                          ...
                          'Неожиданности дают мне возможность по-новому взглянуть на привычные вещи',
        ...
        # Основные шкалы
        part_df['ОЛРО_ВГБ_СТ_К_Значение'] = base_df['К_Значение']
        part_df['ОЛРО_ВГБ_СТ_К_Диапазон'] = base_df['К_Диапазон']

        part_df['ОЛРО_ВГБ_СТ_ЛР_Значение'] = base_df['ЛР_Значение']
        part_df['ОЛРО_ВГБ_СТ_ЛР_Диапазон'] = base_df['ЛР_Уровень']
        ...
        if len(lst_svod_cols) == 0:
            ...
            out_dct = create_result_orlo_vas_st(base_df, out_dct, lst_svod_cols)
            return out_dct, part_df

    except BadOrderOLROVST:
        messagebox.showerror('Лахеcис',
                             f'При обработке вопросов теста Скрининговая методика оценки личностных ресурсов обучающихся Васягина, Григорян, Баринова форма 2 для 9-11 классов, студентов  обнаружены неправильные вопросы. Проверьте названия колонок с вопросами:\n'
                             f'{error_order_message[:5000]}\n'
                             f'Используйте при создании Яндекс-формы написание вопросов из руководства пользователя программы Лахесис.')
    except BadValueOLROVST:
        messagebox.showerror('Лахеcис',
                             f'При обработке вопросов теста Скрининговая методика оценки личностных ресурсов обучающихся Васягина, Григорян, Баринова форма 2 для 9-11 классов, студентов  обнаружены неправильные варианты ответов. Проверьте ответы на указанных строках:\n'
                             f'{error_message[:5000]}\n'
                             f'Используйте при создании Яндекс-формы написание вариантов ответа из руководства пользователя программы Лахесис.')


    except BadCountColumnsOLROVST:
        messagebox.showerror('Лахеcис',
                             f'Проверьте количество колонок с ответами на тест Скрининговая методика оценки личностных ресурсов обучающихся Васягина, Григорян, Баринова форма 2 для 9-11 классов, студентов \n'
                             f'Должно быть 51 колонка с ответами')
