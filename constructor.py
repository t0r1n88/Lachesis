...
def processing_dcok(base_df:pd.DataFrame,df:pd.DataFrame,size:int):
    """
    Фугкция для обработки данных ДЦОК
    :return:
    """
    global threshold_complex
    if df.shape[1] - threshold_complex <= 41:
        raise WrongNumberColumn


    answers_df = df.iloc[:, threshold_complex:threshold_complex + size]  # датафрейм с результатами
...
# создаем описание результата
    ...
    send_df = df.iloc[:, :threshold_complex]
    ...
    out_full_df = df.iloc[:,threshold_complex:] # датафрейм с вопросами и результатами без анкетных данных
    out_result_df = send_df.iloc[:,threshold_complex:]

    # После обработки прибавляем к порогу threshold_complex количество колонок в этом тесте
    threshold_complex += size
...
def processing_optl():
    ...
    try:
        ...
        if df.shape[1] - threshold_complex <= 30:
            raise WrongNumberColumn

        base_df = df.iloc[:, :threshold_complex]  # создаем датафрейм с данными не относящимися к тесту
        ...
        answers_df = df.iloc[:, threshold_complex:threshold_complex+DCT_PARAMS_SCRIPT['ОПТЛ'][1]]  # датафрейм с результатами
...
"""
        ...
        send_df.to_excel(f'{path_to_end_folder_complex}/Краткая таблица с результатами ОТПЛ  от {current_time}.xlsx',
                         index=False, engine='xlsxwriter')
        # добавляем к пределу колонки
        threshold_complex += DCT_PARAMS_SCRIPT['ОПТЛ'][1]
...
def processing_complex():
    ...
    for name_test in lst_tests:
        temp_full_df,temp_result_df = DCT_PARAMS_SCRIPT[name_test][0](base_df_for_func,df,DCT_PARAMS_SCRIPT[name_test][1])
        base_df = pd.concat([base_df,temp_full_df],axis=1) # соединяем анкетные данные и вопросы вместе с результатами
    ...
    base_df.to_excel(f'{path_to_end_folder_complex}/Полная таблица с результатами комплексного теста от {current_time}.xlsx', index=False,
                engine='xlsxwriter')
