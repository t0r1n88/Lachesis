...
def create_result_wcq_nipnib(base_df:pd.DataFrame, out_dct:dict, lst_svod_cols:list):
    ...
    dct_rename_cols_mean = {'К_Значение': 'Ср. Конфронтация',
                            'Д_Значение': 'Ср. Дистанцирование',
                            'С_Значение': 'Ср. Самоконтроль',
                            'ПСП_Значение': 'Ср. Поиск социальной поддержки',

                            'ПО_Значение': 'Ср. Принятие ответственности',
                            'БИ_Значение': 'Ср. Бегство-избегание',
                            'ПРП_Значение': 'Ср. Планирование решения проблемы',
                            'ПП_Значение': 'Ср. Положительная переоценка',
                            }
    ...
    out_dct.update({f'Ср {out_name}': svod_mean_one_df,
                    ...
                    f'ПО {out_name}': svod_count_one_level_po_df,
                    f'БИ {out_name}': svod_count_one_level_bi_df,
                    f'ПРП {out_name}': svod_count_one_level_prp_df,
                    f'ПП {out_name}': svod_count_one_level_pp_df,
                    })
    ...
    if len(lst_svod_cols) == 1:
        ...
        for idx, name_column in enumerate(lst_svod_cols):
            lst_reindex_column_level_cols = [lst_svod_cols[idx],'редкое использование', 'умеренное использование', 'выраженное использование', 'высокий показатель',
                                                  'Итого']
            ...
            out_dct.update({f'Ср {name_column}': svod_mean_column_df,
                            ...
                            f'ПО {name_column}': svod_count_column_level_po_df,
                            f'БИ {name_column}': svod_count_column_level_bi_df,
                            f'ПРП {name_column}': svod_count_column_level_prp_df,
                            f'ПП {name_column}': svod_count_column_level_pp_df,
                            })
        return out_dct
...
def processing_lazarus_wcq_nipni(base_df: pd.DataFrame, answers_df: pd.DataFrame,lst_svod_cols:list):
    ...
    try:
        ...
        lst_prp = list(map(lambda x: x - 1, lst_prp))
        base_df['ПРП_сырое'] = answers_df.take(lst_prp, axis=1).apply(calc_value, axis=1)
        dct_prp = {'Женский':{'до 20 лет':{0:13,1:16,2:19,
                                         3:22,4:25,5:28,
                                         6:32,7:35,8:38,
                                         9:41,10:44,11:47,
                                         12:50,13:53,14:57,
                 ...
                 'Мужской': {'до 20 лет': {0:11 , 1:14 , 2:17 ,
                                           3:20 , 4:24 , 5:27 ,
                                           6:30 , 7:33 , 8:37 ,
                                           9:40 , 10:43 , 11:46 ,
                                           12:49 , 13:53 , 14:56 ,
        ...
        lst_pp = list(map(lambda x: x - 1, lst_pp))
        base_df['ПП_сырое'] = answers_df.take(lst_pp, axis=1).apply(calc_value, axis=1)
        dct_pp = {'Женский':{'до 20 лет':{0:18,1:21,2:24,
                                         3:26,4:29,5:32,
                                         6:34,7:37,8:40,
                                         9:42,10:45,11:48,
                                         12:50,13:53,14:56,
    ...
    except BadOrderWCQLB:
        messagebox.showerror('Лахеcис',
                             f'При обработке вопросов теста Способы совладающего поведения Лазарус НИПНИ Бехтерева обнаружены неправильные вопросы. Проверьте названия колонок с вопросами:\n'
                             f'{error_order_message[:5000]}\n'
                             f'Используйте при создании Яндекс-формы написание вопросов из руководства пользователя программы Лахесис.')
    except BadValueWCQLB:
        messagebox.showerror('Лахеcис',
                             f'При обработке вопросов теста Способы совладающего поведения Лазарус НИПНИ Бехтерева обнаружены неправильные варианты ответов. Проверьте ответы на указанных строках:\n'
                             f'{error_message[:5000]}\n'
                             f'Используйте при создании Яндекс-формы написание вариантов ответа из руководства пользователя программы Лахесис.')


    except BadCountColumnsWCQLB:
        messagebox.showerror('Лахеcис',
                             f'Проверьте количество колонок с ответами на тест Способы совладающего поведения Лазарус НИПНИ Бехтерева\n'
                             f'Должно быть 50 колонок с ответами')
        ...
        messagebox.showerror('Лахеcис',
                             ...
                             f'Прочитайте страницу 1 и 2 файла инструкции к тесту Способы совладающего поведения, WCQ Лазарус')
    except BadValueAge:
        messagebox.showerror('Лахеcис',
                             f'В колонке Возраст найдены значения отличающиеся от допустимых {diff_age}\n'
                             f'Допускаются значения: до 20 лет, 21-30 лет, 31-45 лет, 46-60 лет\n'
                             f'Прочитайте страницу 1 и 2 файла инструкции к тесту Способы совладающего поведения, WCQ Лазарус')
