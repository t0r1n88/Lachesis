...
от 8 до 12 баллов – ярко выраженный интерес.\n
        ...
        """
            Своды 
            """
        lst_reindex_group_cols = ['Группа','область деятельности активно отрицается', 'область деятельности интереса не вызывает', 'интерес выражен слабо',
                   'выраженный интерес','ярко выраженный интерес' ,'Итого']
        lst_reindex_group_sex_cols = ['Группа','Пол','область деятельности активно отрицается', 'область деятельности интереса не вызывает', 'интерес выражен слабо',
                   'выраженный интерес','ярко выраженный интерес' ,'Итого']

        lst_reindex_course_cols = ['Курс','область деятельности активно отрицается', 'область деятельности интереса не вызывает', 'интерес выражен слабо',
                   'выраженный интерес','ярко выраженный интерес' ,'Итого']
        lst_reindex_course_sex_cols = ['Курс','Пол','область деятельности активно отрицается', 'область деятельности интереса не вызывает', 'интерес выражен слабо',
                   'выраженный интерес','ярко выраженный интерес' ,'Итого']

        # Своды по уровням
        # Группа
        svod_group_level_df = calc_mean(base_df, 'Группа', ['Группа', 'Уровень'], 'Максимум')
        svod_count_group_level_df = calc_count_level_miga(base_df, 'Группа', ['Группа'], 'Максимум', 'Уровень',
                                                          lst_reindex_group_cols)

        # Группа Пол
        svod_group_level_sex_df = calc_mean(base_df, 'Группа', ['Группа', 'Уровень', 'Пол'], 'Максимум')
        svod_count_group_level_sex_df = calc_count_level_miga(base_df, 'Группа', ['Группа', 'Пол'], 'Максимум', 'Уровень',
                                                              lst_reindex_group_sex_cols)

        # Курс
        svod_course_level_df = calc_mean(base_df, 'Курс', ['Курс', 'Уровень'], 'Максимум')
        svod_count_course_level_df = calc_count_level_miga(base_df, 'Курс', ['Курс'], 'Максимум',
                                                           'Уровень', lst_reindex_course_cols)

        # Курс Пол
        svod_course_level_sex_df = calc_mean(base_df, 'Курс', ['Курс', 'Уровень', 'Пол'], 'Максимум')
        svod_count_course_level_sex_df = calc_count_level_miga(base_df, 'Курс', ['Курс', 'Пол'],
                                                               'Максимум',
        ...
        svod_count_group_sphere_df = calc_count_sphere_miga(base_df, 'Группа', ['Группа'], 'Максимум', 'Обработанное')

        # Группа Пол
        svod_group_sphere_sex_df = calc_mean(base_df,'Группа',['Группа','Обработанное','Пол'],'Максимум')
        svod_count_group_sphere_sex_df = calc_count_sphere_miga(base_df, 'Группа', ['Группа', 'Пол'], 'Максимум', 'Обработанное')

        # Курс
        svod_course_sphere_df = calc_mean(base_df,'Курс',['Курс','Обработанное'],'Максимум')
        svod_count_course_sphere_df = calc_count_sphere_miga(base_df, 'Курс', ['Курс'], 'Максимум', 'Обработанное')

        # Курс Пол
        svod_course_sphere_sex_df = calc_mean(base_df,'Курс',['Курс','Обработанное','Пол'],'Максимум')
        svod_count_course_sphere_sex_df = calc_count_sphere_miga(base_df, 'Курс', ['Курс', 'Пол'], 'Максимум', 'Обработанное')
        ...
        messagebox.showerror('Лахеcис',
                             f'При обработке вопросов теста Карта интересов Голомшток обнаружены неправильные вопросы. Проверьте названия колонок с вопросами:\n'
                             f'{error_order_message}\n'
                             f'Используйте при создании Яндекс-формы написание вопросов из руководства пользователя программы Лахесис.')
    except BadValueMIGA:
        messagebox.showerror('Лахеcис',
                             f'При обработке вопросов теста Карта интересов Голомшток обнаружены неправильные варианты ответов. Проверьте ответы на указанных строках:\n'
                             f'{error_message}\n'
        ...
        messagebox.showerror('Лахеcис',
                             f'Проверьте количество колонок с ответами на тест Карта интересов Голомшток\n'
