"""
Скрипт для обработки результатов теста тревожности Кондаша для студентов
"""
...
class NotReqColumn(Exception):
    """
    Исключение для обработки случая когда нет обязательных колонок Курс, Группа, Пол
    """
    pass
...
def create_result_kondash(base_df:pd.DataFrame, out_dct:dict,):
    ...
    :return: словарь
    """
    lst_reindex_one_level_cols = ['Курс','Группа','Пол','нормальный уровень тревожности', 'несколько повышенный уровень тревожности',
             'высокий уровень тревожности','очень высокий уровень тревожности','чрезмерное спокойствие',
                                                      'Итого']

    lst_kondash_svod_cols = ['Курс','Группа','Пол']
    ...
    out_dct.update({f'Ср Курс Группа Пол': svod_mean_one_df,
                    f'Свод Курс Группа Пол ОТ': svod_count_one_level_ot_df,
                    f'Свод Курс Группа Пол УТ': svod_count_one_level_ut_df,
                    f'Свод Курс Группа Пол СТ': svod_count_one_level_st_df,
                    f'Свод Курс Группа Пол МТ': svod_count_one_level_mt_df})
...
def processing_kondash_anxiety_student(base_df: pd.DataFrame, answers_df: pd.DataFrame,lst_svod_cols:list):
    """
    Проверка колонок и значений таблицы
    """
    try:
        # Проверяем наличие колонок Курс, Группа, Пол
        diff_req_cols = {'Курс','Группа','Пол'}.difference(set(base_df.columns))
        if len(diff_req_cols) !=0:
            raise NotReqColumn

        base_df['Курс'] = base_df['Курс'].apply(convert_to_int)
        base_df['Курс'] = base_df['Курс'].astype(str)
        base_df['Группа'] = base_df['Группа'].astype(str)
        # Проверяем на пол
        ...
        lst_check_cols = ['Отвечать у доски', 'Пойти в дом к незнакомым людям',
                          ...
                          'Разговаривать с директором техникума, колледжа', 'Думать о своем будущем',
                          'Преподаватель смотрит по журналу, кого бы спросить',
                          ...
                          'После контрольной преподаватель называет отметки', 'На тебя не обращают внимания',
                          ...
                          'Сдаешь экзамены в техникуме, колледже', 'На тебя сердятся (непонятно почему)',
                          'Выступать перед большой аудиторией',
                          'Предстоит важное, решающее дело', 'Не понимаешь объяснений преподавателя',
                          'С тобой не согласны, противоречат тебе',
                          'Сравниваешь себя с другими', 'Проверяют твои способности',
                          'На тебя смотрят как на маленького',
                          'На паре преподаватель неожиданно задает тебе вопрос', 'Замолчали, когда ты подошел',
        ...
        base_df['Уровень_общей_тревожности'] = base_df[['Курс', 'Пол', 'Значение_общей_тревожности']].apply(
        ...
        base_df['Норма_самоценочной_тревожности'] = base_df[['Курс', 'Пол']].apply(calc_norm_self_condash_anxiety,
                                                                                   axis=1)
        ...
        base_df['Норма_межличностной_тревожности'] = base_df[['Курс', 'Пол']].apply(calc_norm_soc_condash_anxiety,
                                                                                    axis=1)
