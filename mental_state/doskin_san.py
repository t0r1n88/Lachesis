...
def calc_count_level_sub(df:pd.DataFrame, lst_cat:list, val_cat, col_cat, lst_cols:list):
    ...
    count_df['% не благоприятное состояние от общего'] = round(
        count_df['не благоприятное состояние'] / count_df['Итого'], 2) * 100
    count_df['% нормальное состояние от общего'] = round(
        count_df['нормальное состояние'] / count_df['Итого'], 2) * 100
    count_df['% благоприятное состояние от общего'] = round(
        count_df['благоприятное состояние'] / count_df['Итого'], 2) * 100

    return count_df


def calc_count_level_var_two(df:pd.DataFrame, lst_cat:list, val_cat, col_cat, lst_cols:list):
    """
    Функция для создания сводных датафреймов по субшкалам
    ...
    count_df.reset_index(inplace=True)
    count_df = count_df.reindex(columns=lst_cols)
    count_df['% преобладает плохое настроение от общего'] = round(
        count_df['преобладает плохое настроение'] / count_df['Итого'], 2) * 100
    count_df['% между плохим и изменчивым от общего'] = round(
        count_df['между плохим и изменчивым'] / count_df['Итого'], 2) * 100
    count_df['% изменчивое настроение от общего'] = round(
        count_df['изменчивое настроение'] / count_df['Итого'], 2) * 100
    count_df['% между изменчивым и хорошим от общего'] = round(
        count_df['между изменчивым и хорошим'] / count_df['Итого'], 2) * 100
    count_df['% преобладает хорошее настроение от общего'] = round(
        count_df['преобладает хорошее настроение'] / count_df['Итого'], 2) * 100
...
def create_result_dsan(base_df:pd.DataFrame, out_dct:dict, lst_svod_cols:list):
    ...
    lst_reindex_main_level_cols.extend( ['не благоприятное состояние', 'нормальное состояние', 'благоприятное состояние',
                                   'Итого'])  # Основная шкала


    lst_reindex_sub_level_cols = lst_svod_cols.copy()
    lst_reindex_sub_level_cols.extend( ['преобладает плохое настроение', 'между плохим и изменчивым', 'изменчивое настроение',
               'между изменчивым и хорошим', 'преобладает хорошее настроение',
                                   'Итого'])  # Субшкалы

    # САН
    svod_count_one_level_well_being_df = calc_count_level_sub(base_df, lst_svod_cols,
                                                      'Значение_шкалы_Самочувствие',
                                                      'Уровень_шкалы_Самочувствие',
                                                      lst_reindex_main_level_cols)
    svod_count_one_level_activ_df = calc_count_level_sub(base_df, lst_svod_cols,
                                                      'Значение_шкалы_Активность',
                                                      'Уровень_шкалы_Активность',
                                                      lst_reindex_main_level_cols)
    svod_count_one_level_mood_df = calc_count_level_sub(base_df, lst_svod_cols,
                                                      'Значение_шкалы_Настроение',
                                                      'Уровень_шкалы_Настроение',
                                                      lst_reindex_main_level_cols)

    # Вариант 2
    svod_count_one_level_var_two_df = calc_count_level_var_two(base_df, lst_svod_cols,
                                                      'Значение_Вариант_два',
                                                      'Уровень_Вариант_два',
    ...
    svod_mean_df = pd.pivot_table(base_df,
                                  index=lst_svod_cols,
                                  values=['Значение_шкалы_Самочувствие',
                                          'Значение_шкалы_Активность',
                                          'Значение_шкалы_Настроение',
                                          'Значение_Вариант_два',
                                          ],
    ...
    new_order_cols.extend((['Значение_шкалы_Самочувствие', 'Значение_шкалы_Активность',
                            'Значение_шкалы_Настроение', 'Значение_Вариант_два',
                            ]))
    svod_mean_df = svod_mean_df.reindex(columns=new_order_cols)

    dct_rename_cols_mean = {'Значение_шкалы_Самочувствие': 'Ср. Самочувствие',
                            'Значение_шкалы_Активность': 'Ср. Активность',
                            'Значение_шкалы_Настроение': 'Ср. Настроение',
                            'Значение_Вариант_два': 'Ср. Вариант_два'}
    ...
    out_dct.update({f'Ср {out_name}': svod_mean_df,
                    f'Свод С {out_name}': svod_count_one_level_well_being_df,
                    f'Свод А {out_name}': svod_count_one_level_activ_df,
                    f'Свод Н {out_name}': svod_count_one_level_mood_df,
                    f'Свод В2 {out_name}': svod_count_one_level_var_two_df})

    if len(lst_svod_cols) == 1:
        return out_dct
    else:
        for idx, name_column in enumerate(lst_svod_cols):
            lst_reindex_column_main_level_cols = [lst_svod_cols[idx],'не благоприятное состояние', 'нормальное состояние', 'благоприятное состояние',
                                   'Итого']  # Основная шкала

            lst_reindex_column_var_two_level_cols = [lst_svod_cols[idx],'преобладает плохое настроение', 'между плохим и изменчивым', 'изменчивое настроение',
               'между изменчивым и хорошим', 'преобладает хорошее настроение',
                                   'Итого']  # Основная шкала
            # САН
            svod_count_column_level_well_being_df = calc_count_level_sub(base_df, lst_svod_cols[idx],
                                                                      'Значение_шкалы_Самочувствие',
                                                                      'Уровень_шкалы_Самочувствие',
                                                                      lst_reindex_column_main_level_cols)
            svod_count_column_level_activ_df = calc_count_level_sub(base_df, lst_svod_cols[idx],
                                                                 'Значение_шкалы_Активность',
                                                                 'Уровень_шкалы_Активность',
                                                                 lst_reindex_column_main_level_cols)
            svod_count_column_level_mood_df = calc_count_level_sub(base_df, lst_svod_cols[idx],
                                                                'Значение_шкалы_Настроение',
                                                                'Уровень_шкалы_Настроение',
                                                                lst_reindex_column_main_level_cols)

            # Вариант 2
            svod_count_column_level_var_two_df = calc_count_level_var_two(base_df, lst_svod_cols[idx],
                                                                       'Значение_Вариант_два',
                                                                       'Уровень_Вариант_два',
                                                                       lst_reindex_column_var_two_level_cols)
            ...
            out_dct.update({f'Ср {name_column}': svod_mean_column_df,
                            f'Свод С {name_column}': svod_count_column_level_well_being_df,
                            f'Свод А {name_column}': svod_count_column_level_activ_df,
                            f'Свод Н {name_column}': svod_count_column_level_mood_df,
                            f'Свод В2 {name_column}': svod_count_column_level_var_two_df})
        return out_dct
...
def processing_doskin_san(result_df: pd.DataFrame, answers_df: pd.DataFrame, lst_svod_cols:list):
    ...
    try:
        ...
        dct_prefix = {'Уровень_шкалы_Самочувствие': 'С',
                      'Уровень_шкалы_Активность': 'А',
                      'Уровень_шкалы_Настроение': 'Н',
                      }
        ...
        for level in lst_var_two:
            temp_df = base_df[base_df['Уровень_Вариант_два'] == level]
            if temp_df.shape[0] != 0:
                if level == 'преобладает плохое настроение':
                    level = 'плохое настроение'
                elif level == 'преобладает хорошее настроение':
                    level = 'хорошее настроение'
        ...
        messagebox.showerror('Лахеcис',
                             f'При обработке вопросов теста Самочувствие-Активность-Настроение Доскин Мирошников обнаружены неправильные варианты ответов. Проверьте ответы на указанных строках:\n'
                             f'{error_message[:5000]}\n'
                             f'Используйте при создании Яндекс-формы написание вариантов ответа из руководства пользователя программы Лахесис.')


    except BadCountColumnsDSAN:
        messagebox.showerror('Лахеcис',
                             f'Проверьте количество колонок с ответами на тест Самочувствие-Активность-Настроение Доскин Мирошников\n'
