...
from lachesis_support_functions import round_mean, sort_name_class,create_union_svod,create_list_on_level
...
def calc_count_sphere(df:pd.DataFrame, lst_cat:list, val_cat, col_cat, lst_cols:list):
    ...
    count_df.reset_index(inplace=True)
    count_df = count_df.reindex(columns=lst_cols)
    count_df['% Реалистический от общего'] = round(
        count_df['Реалистический'] / count_df['Итого'], 2) * 100
    count_df['% Интеллектуальный от общего'] = round(
        count_df['Интеллектуальный'] / count_df['Итого'], 2) * 100
    count_df['% Социальный от общего'] = round(
        count_df['Социальный'] / count_df['Итого'], 2) * 100
    count_df['% Офисный от общего'] = round(
        count_df['Офисный'] / count_df['Итого'], 2) * 100
    count_df['% Предпринимательский от общего'] = round(
        count_df['Предпринимательский'] / count_df['Итого'], 2) * 100
    count_df['% Артистический от общего'] = round(
        count_df['Артистический'] / count_df['Итого'], 2) * 100
...
def create_result_holland_ptl(base_df:pd.DataFrame, out_dct:dict, lst_svod_cols:list):
    ...
    lst_reindex_one_level_cols.extend( ['слабо выраженный тип', 'средне выраженный тип', 'ярко выраженный тип',
                                                      'Итого'])

    lst_reindex_one_sphere_cols = lst_svod_cols.copy()
    lst_reindex_one_sphere_cols.extend( ['Реалистический', 'Интеллектуальный', 'Социальный', 'Офисный',
              'Предпринимательский', 'Артистический','Итого'])

    svod_count_one_level_df = calc_count_level(base_df, lst_svod_cols,
                                               'Значение_ведущий_тип_личности',
                                               'Уровень_выраженности',
                                               lst_reindex_one_level_cols)

    svod_count_one_sphere_df = calc_count_sphere(base_df, lst_svod_cols,
                                                 'Значение_ведущий_тип_личности',
                                                 'Ведущий_тип_личности',
                                                 lst_reindex_one_sphere_cols)

    # Считаем среднее по субшкалам
    svod_mean_one_df = pd.pivot_table(base_df,
                                      index=lst_svod_cols,
                                      values=['Значение_ведущий_тип_личности',
                                              ],
                                      aggfunc=round_mean)
    svod_mean_one_df.reset_index(inplace=True)
    # упорядочиваем колонки
    new_order_cols = lst_svod_cols.copy()
    new_order_cols.extend((['Значение_ведущий_тип_личности',
                            ]))
    svod_mean_one_df = svod_mean_one_df.reindex(columns=new_order_cols)

    dct_rename_cols_mean = {'Значение_ведущий_тип_личности': 'Среднее значение ведущего типа личности',
                            }
    ...
    if len(lst_svod_cols) == 1:
        ...
        for idx, name_column in enumerate(lst_svod_cols):
            lst_reindex_column_level_cols = [lst_svod_cols[idx], 'слабо выраженный тип', 'средне выраженный тип', 'ярко выраженный тип',
                                             'Итого']

            lst_reindex_column_sphere_cols = [lst_svod_cols[idx], 'Реалистический', 'Интеллектуальный', 'Социальный', 'Офисный',
              'Предпринимательский', 'Артистический','Итого']
            ...
            dct_rename_cols_mean = {
                'Значение_ведущий_тип_личности': 'Среднее значение ведущего типа личности',
                }
...
def processing_holland_ptl(base_df: pd.DataFrame, answers_df: pd.DataFrame,lst_svod_cols:list):
    """
    ...
    try:
        ...
        base_df[f'Распределение'] = answers_df.apply(processing_result_ptl, axis=1)
        base_df[f'Ведущий_тип_личности'] = base_df[f'Распределение'].apply(
            extract_key_max_value)
        base_df[f'Значение_ведущий_тип_личности'] = base_df[f'Распределение'].apply(
            extract_max_value)
        base_df[f'Уровень_выраженности'] = base_df[f'Значение_ведущий_тип_личности'].apply(
            calc_level_ptl)

        # Создаем датафрейм для создания части в общий датафрейм
        part_df = pd.DataFrame()
        part_df['ПТЛ_Распределение'] = base_df['Распределение']
        part_df['ПТЛ_Ведущий_тип'] = base_df['Ведущий_тип_личности']
        part_df['ПТЛ_Тип_Значение'] = base_df['Значение_ведущий_тип_личности']
        part_df['ПТЛ_Тип_Уровень'] = base_df['Уровень_выраженности']

        base_df.sort_values(by='Значение_ведущий_тип_личности', ascending=False, inplace=True)  # сортируем
        ...
        # Списки для шкал
        lst_sphere = ['Реалистический', 'Интеллектуальный', 'Социальный', 'Офисный',
        ...
        avg_main = round(base_df['Значение_ведущий_тип_личности'].mean(), 2)

        avg_dct = {'Среднее значение Ведущий тип личности': avg_main,
                   }
        ...
        for level in lst_level:
            ...
            if temp_df.shape[0] != 0:
                if level == 'слабо выраженный тип':
                    level = 'слабо выраженный'
                elif level == 'средне выраженный тип':
                    level = 'средне выраженный'
        ...
        for sphere in lst_sphere:
            temp_df = base_df[base_df['Ведущий_тип_личности'] == sphere]
            if temp_df.shape[0] != 0:
