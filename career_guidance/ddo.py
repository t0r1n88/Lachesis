...
class WrongNumberColumn(Exception):
    """
    Класс для исключения проверяющего количество колонок в таблице
    """
    pass
...
def processing_ddo(base_df: pd.DataFrame, answers_df: pd.DataFrame, size: int,name_test,threshold_base:int,end_folder:str):
    """
    ...
    answers_df[f'{name_test}_Числовое_значение_результата'] = answers_df[f'{name_test}_Необработанный_результат'].apply(
        extract_max_value)
    answers_df[f'{name_test}_Описание_результата'] = answers_df[f'{name_test}_Обработанный_результат'].apply(lambda x:create_out_str_ddo(x,dct_desciprion,dct_prof))
