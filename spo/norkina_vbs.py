...
def processing_result_vbs(row):
    ...
    if row[2] == 'да, один или два':
        ...
        dct_type['наблюдатель'] += 1
    elif row[2] == 'да, но они не приятны всем в группе':
        dct_type['инициатор'] += 1
        dct_type['помощник'] += 1
    ...
    if row[7] == 'облегчение, хорошо, что меня это не касается':
        dct_type['наблюдатель'] += 1
    elif row[7] == 'несправедливость и заступаюсь за одногруппника':
        dct_type['защитник'] += 1
    ...
    if row[13] == 'да, мне не нравится наш коллектив':
        ...
        dct_type['инициатор'] += 1
        dct_type['помощник'] += 1
    elif row[13] == 'иногда, после ссоры с одногруппниками':
        dct_type['защитник'] += 1
    ...
    if row[23] == 'да, для меня это очень важно':
        ...
        dct_type['инициатор'] += 1
    elif row[23] == 'нет, я никогда не был успешен в группе':
        dct_type['жертва'] += 1
...
def processing_vbs(base_df: pd.DataFrame, answers_df: pd.DataFrame):
    ...
    try:
        ...
        lst_check_cols = ['Среди одногруппников у меня много друзей',
                          'Для меня важна внешность окружающих',
                          'В моей группе есть ребята, которые мне не приятны',
                          'Мне кажется, что мои одногруппники лучше меня',
                          'Если мой одногруппник пришел в очках',
                          'У меня очень дружная группа',
                          'Я часто испытываю чувство одиночества и тревоги',
                          'Если при мне обижают одногруппника, я чувствую',
                          'Я бы хотел проводить больше времени с одногруппниками',
                          'Мне кажется, что мои одногруппники не хотят со мной общаться',
                          'У нас в группе есть пару ребят, на которых все равняются',
                          'Когда меня ругают, я испытываю чувство гнева',
                          'В нашей группе есть несколько ребят, которых все боятся',
                          'Мне бы хотелось учиться в другой группе или техникуме, колледже',
                          'Мне кажется, что с помощью силы можно решить любую проблему',
                          'В моей группе есть один (несколько) человек, с которыми никто не дружит',
                          'Мне кажется, что в нашей группе часто происходят акты насилия (обзывания, насмешки, обидные жесты или действия)',
                          'Если я увижу драку между одногруппниками, то я',
                          'Мне кажется, что в коллективе меня недооценивают',
                          'По-моему, педагоги в техникуме, колледже унижают и оскорбляют учащихся',
                          'Если бы моя группа была на корабле, я бы стал',
                          'Если у человека изъяны во внешности (бородавки, косоглазие, ожирение и др.)',
                          'Если при мне кто-то подвергается насмешкам',
                          'Я часто огорчаюсь, когда не пользуюсь успехом в группе',
                          'Я нуждаюсь в поддержке одногруппников',
                          ]
        ...
        valid_values = [['да, я дружу со всеми','у меня есть пару друзей','нет, я ни с кем не дружу','мне бы хотелось дружить со всеми'],
                        ...
                        ['да','нет','иногда','часто'],
                        ['облегчение, хорошо, что меня это не касается','несправедливость и заступаюсь за одногруппника','ничего не чувствую, наверняка он это заслужил','мне нет до этого никакого дела'],
                        ...
                        ['да, мне не нравится наш коллектив','нет, меня все устраивает','иногда, после ссоры с одногруппниками','нет, а вдруг там будет хуже'],
                        ...
                        ['да, для меня это очень важно','нет, мне все равно','я всегда пользуюсь успехом','нет, я никогда не был успешен в группе'],
                        ['да','нет','иногда','часто']
                        ]
        ...
        base_df['Вопрос_13'] = answers_df['В нашей группе есть несколько ребят, которых все боятся']
        base_df['Вопрос_17'] = answers_df['Мне кажется, что в нашей группе часто происходят акты насилия (обзывания, насмешки, обидные жесты или действия)']
        base_df['Вопрос_20'] = answers_df['По-моему, педагоги в техникуме, колледже унижают и оскорбляют учащихся']
        ...
        """
            Своды 
            """
        lst_reindex_group_cols = ['Группа', 'инициатор', 'помощник',
                   'защитник',
                   'жертва', 'наблюдатель', 'Итого']
        lst_reindex_group_sex_cols = ['Группа', 'Пол', 'инициатор', 'помощник',
                   'защитник',
                   'жертва', 'наблюдатель', 'Итого']

        lst_reindex_course_cols = ['Курс', 'инициатор', 'помощник',
                   'защитник',
                   'жертва', 'наблюдатель', 'Итого']
        lst_reindex_course_sex_cols = ['Курс', 'Пол', 'инициатор', 'помощник',
                   'защитник',
                   'жертва', 'наблюдатель', 'Итого']

        # Группа
        svod_group_level_df = calc_mean(base_df, 'Группа', ['Группа', 'Роль'], 'Числовое_значение_роли')
        svod_count_group_level_df = calc_count_level_spp(base_df, 'Группа', ['Группа'], 'Числовое_значение_роли', 'Роль',
                                                         lst_reindex_group_cols)

        # Группа Пол
        svod_group_level_sex_df = calc_mean(base_df, 'Группа', ['Группа', 'Роль', 'Пол'], 'Числовое_значение_роли')
        svod_count_group_level_sex_df = calc_count_level_spp(base_df, 'Группа', ['Группа', 'Пол'], 'Числовое_значение_роли', 'Роль',
                                                             lst_reindex_group_sex_cols)

        # Курс
        svod_course_level_df = calc_mean(base_df, 'Курс', ['Курс', 'Роль'], 'Числовое_значение_роли')
        svod_count_course_level_df = calc_count_level_spp(base_df, 'Курс', ['Курс'], 'Числовое_значение_роли',
                                                          'Роль', lst_reindex_course_cols)

        # Курс Пол
        svod_course_level_sex_df = calc_mean(base_df, 'Курс', ['Курс', 'Роль', 'Пол'], 'Числовое_значение_роли')
        ...
        svod_dct = {'Ср. Роль Группа': svod_group_level_df, 'Кол. Роль Группа': svod_count_group_level_df,
                    'Ср. Роль Группа Пол': svod_group_level_sex_df,'Кол. Роль Группа Пол': svod_count_group_level_sex_df,
                    'Ср. Роль Курс': svod_course_level_df,'Кол. Роль Курс': svod_count_course_level_df,
                    'Ср. Роль Курс Пол': svod_course_level_sex_df,'Кол. Роль Курс Пол': svod_count_course_level_sex_df,
        ...
        # Своды по вопросу 13
        # Группа
        thirteen_svod_count_group_level_df = calc_count_sphere_spp(base_df, 'Группа', ['Группа'], 'Числовое_значение_роли', 'Вопрос_13')

        # Группа Пол
        thirteen_svod_count_group_level_sex_df = calc_count_sphere_spp(base_df, 'Группа', ['Группа', 'Пол'], 'Числовое_значение_роли',
                                                                    'Вопрос_13')

        # Курс
        thirteen_svod_count_course_level_df = calc_count_sphere_spp(base_df, 'Курс', ['Курс'], 'Числовое_значение_роли',
                                                                 'Вопрос_13')

        # Курс Пол
        thirteen_svod_count_course_level_sex_df = calc_count_sphere_spp(base_df, 'Курс', ['Курс', 'Пол'],
                                                                     'Числовое_значение_роли', 'Вопрос_17')

        # Своды по вопросу 17
        # Группа
        seventeen_svod_count_group_level_df = calc_count_sphere_spp(base_df, 'Группа', ['Группа'], 'Числовое_значение_роли', 'Вопрос_17')

        # Группа Пол
        seventeen_svod_count_group_level_sex_df = calc_count_sphere_spp(base_df, 'Группа', ['Группа', 'Пол'], 'Числовое_значение_роли',
                                                                    'Вопрос_17')

        # Курс
        seventeen_svod_count_course_level_df = calc_count_sphere_spp(base_df, 'Курс', ['Курс'], 'Числовое_значение_роли',
                                                                 'Вопрос_17')

        # Курс Пол
        seventeen_svod_count_course_level_sex_df = calc_count_sphere_spp(base_df, 'Курс', ['Курс', 'Пол'],
                                                                     'Числовое_значение_роли', 'Вопрос_17')


        # Своды по вопросу 20
        # Группа
        twenty_svod_count_group_level_df = calc_count_sphere_spp(base_df, 'Группа', ['Группа'], 'Числовое_значение_роли', 'Вопрос_20')

        # Группа Пол
        twenty_svod_count_group_level_sex_df = calc_count_sphere_spp(base_df, 'Группа', ['Группа', 'Пол'], 'Числовое_значение_роли',
                                                                    'Вопрос_20')

        # Курс
        twenty_svod_count_course_level_df = calc_count_sphere_spp(base_df, 'Курс', ['Курс'], 'Числовое_значение_роли',
                                                                 'Вопрос_20')

        # Курс Пол
        twenty_svod_count_course_level_sex_df = calc_count_sphere_spp(base_df, 'Курс', ['Курс', 'Пол'],
                                                                     'Числовое_значение_роли', 'Вопрос_20')



        answers_dct = {'Срез по вопросу 13 Группа':thirteen_svod_count_group_level_df,
                       'Срез по вопросу 13 Группа Пол': thirteen_svod_count_group_level_sex_df,
                       ...
                       'Срез по вопросу 17 Группа': seventeen_svod_count_group_level_df,
                       'Срез по вопросу 17 Группа Пол': seventeen_svod_count_group_level_sex_df,
                       ...
                       'Срез по вопросу 20 Группа': twenty_svod_count_group_level_df,
                       'Срез по вопросу 20 Группа Пол': twenty_svod_count_group_level_sex_df,
